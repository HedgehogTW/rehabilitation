// SignalAnalysis.cpp : 定義主控台應用程式的進入點。
//
...
char* tmp_1_filename = "C:\\Users\\ChiaHao\\Documents\\Visual Studio 2013\\Projects\\SignalAnalysis\\SignalAnalysis\\肩胛骨_Ver.4.csv";
char* tmp_2_filename = "C:\\Users\\ChiaHao\\Documents\\Visual Studio 2013\\Projects\\SignalAnalysis\\SignalAnalysis\\拉毛巾_Ver.4.csv";
char* tmp_3_filename = "C:\\Users\\ChiaHao\\Documents\\Visual Studio 2013\\Projects\\SignalAnalysis\\SignalAnalysis\\背後抬手_Ver.4.csv";
char* tmp_4_filename = "C:\\Users\\ChiaHao\\Documents\\Visual Studio 2013\\Projects\\SignalAnalysis\\SignalAnalysis\\鐘擺_Ver.4.csv";
char* tmp_5_filename = "C:\\Users\\ChiaHao\\Documents\\Visual Studio 2013\\Projects\\SignalAnalysis\\SignalAnalysis\\爬牆_Ver.4.csv";
//char* tmp_6_filename = "C:\\Users\\ChiaHao\\Documents\\Visual Studio 2013\\Projects\\SignalAnalysis\\SignalAnalysis\\轉盤_new3.csv";
...
cv::Mat PCA(cv::Mat AfterGSData, char* filename)
{
	...
	for (int i = 0; i < AfterGSData.rows; i++){
		...
		mat_Reflection.at<double>(i, 0) = Sign(Fx); //  12/29  修正 最後填進去MAT需要再取一次sign
	...
	mat_Normailization = (1.0 / s)* mat_Affine; //*mat_NewReflection;  //有問題啊~~~~
...
cv::Mat FeatureExtraction(cv::Mat countFeaturedata)
{
	...
	/*K means cuvature approximation  算錐體得torsion  Center 幾何中心*/
...
//
//	FILE* file1 = fopen("拉毛巾_Peroid.csv", "w");
//
//	for (int i = 0; i < count; i++){
//		if (i%index == 0 && i != 0){
...
/*tmp && src period returns DTW distance*/
{
	...
	/*select Tmplate to matching  取第三(2-3)個周期 */
...
cv::Mat GetIntervalSignal(cv::Mat mat_src, int StartIndex, int EndIndex)
{
	...
	for (int i = StartIndex; i <= EndIndex; i++){
		for (int j = 0; j < mat_src.cols; j++){
			mat_period.at<double>(i - StartIndex, j) = mat_src.at <double>(i, j); //mat_period 要從0開始 
		}
	}
	//tracemat(mat_period);
